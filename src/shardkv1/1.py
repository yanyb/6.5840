...
def run_test(test: str, race: bool, timing: bool):
    test_cmd = ["go", "test", f"-run={test}", f"-timeout=1024s"]
    if race:
...
# fmt: off
def run_tests(
        tests: List[str] = ["OneConcurrentClerkUnreliable5A", ],
        sequential: bool = typer.Option(True, '--sequential', '-s', help='Run all test of each group in order'),
        workers: int = typer.Option(5, '--workers', '-p', help='Number of parallel tasks'),
