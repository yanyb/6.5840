package rsm

import (
	"6.5840/kvsrv1/rpc"
	"6.5840/labrpc"
	"6.5840/raft1"
	"6.5840/raftapi"
	"6.5840/tester1"
	"fmt"
	"sync"
	"sync/atomic"
	"time"
)

var useRaftStateMachine bool // to plug in another raft besided raft1

type Op struct {
	// Your definitions here.
	// Field names must start with capital letters,
	// otherwise RPC will break.
	Id  string
	Req any
}

// A server (i.e., ../server.go) that wants to replicate itself calls
// MakeRSM and must implement the StateMachine interface.  This
// interface allows the rsm package to interact with the server for
// server-specific operations: the server must implement DoOp to
// execute an operation (e.g., a Get or Put request), and
// Snapshot/Restore to snapshot and restore the server's state.
type StateMachine interface {
	DoOp(any) any
	Snapshot() []byte
	Restore([]byte)
}

type RSM struct {
	mu           sync.Mutex
	me           int
	rf           raftapi.Raft
	applyCh      chan raftapi.ApplyMsg
	maxraftstate int // snapshot if log grows this big
	sm           StateMachine
	// Your definitions here.
	lastApplied int
	killedCh    chan struct{}
	resChMap    map[string]chan any
	killed      atomic.Bool
}

// servers[] contains the ports of the set of
// servers that will cooperate via Raft to
// form the fault-tolerant key/value service.
//
// me is the index of the current server in servers[].
//
// the k/v server should store snapshots through the underlying Raft
// implementation, which should call persister.SaveStateAndSnapshot() to
// atomically save the Raft state along with the snapshot.
// The RSM should snapshot when Raft's saved state exceeds maxraftstate bytes,
// in order to allow Raft to garbage-collect its log. if maxraftstate is -1,
// you don't need to snapshot.
//
// MakeRSM() must return quickly, so it should start goroutines for
// any long-running work.
func MakeRSM(servers []*labrpc.ClientEnd, me int, persister *tester.Persister, maxraftstate int, sm StateMachine) *RSM {
	rsm := &RSM{
		me:           me,
		maxraftstate: maxraftstate,
		applyCh:      make(chan raftapi.ApplyMsg),
		sm:           sm,
	}
	if !useRaftStateMachine {
		rsm.rf = raft.Make(servers, me, persister, rsm.applyCh)
	}
	rsm.killedCh = make(chan struct{})
	rsm.resChMap = make(map[string]chan any)
	go rsm.applier()
	go rsm.snapshot()
	return rsm
}

func (rsm *RSM) Raft() raftapi.Raft {
	return rsm.rf
}

func (rsm *RSM) applier() {
	for {
		select {
		case applyMsg, ok := <-rsm.applyCh:
			if !ok {
				close(rsm.killedCh)
				rsm.killed.Store(true)
				return
			}
			if applyMsg.CommandValid {
				op := applyMsg.Command.(Op)
				rsm.mu.Lock()
				res := rsm.sm.DoOp(op.Req)
				rsm.lastApplied = applyMsg.CommandIndex
				resCh := rsm.resChMap[op.Id]
				rsm.mu.Unlock()
				// if me commit the log

				if resCh != nil {
					select {
					case resCh <- res:
						// success
					}
				}
			}

			if applyMsg.SnapshotValid {
				rsm.mu.Lock()
				rsm.sm.Restore(applyMsg.Snapshot)
				rsm.lastApplied = applyMsg.SnapshotIndex
				rsm.mu.Unlock()
			}
		}
	}
}

func (rsm *RSM) snapshot() {
	if rsm.maxraftstate <= 0 {
		return
	}
	ticker := time.NewTicker(time.Millisecond * 10)
	defer ticker.Stop()
	for !rsm.killed.Load() {
		select {
		case <-ticker.C:
			size := rsm.rf.PersistBytes()
			if size >= rsm.maxraftstate {
				rsm.mu.Lock()
				snapshot := rsm.sm.Snapshot()
				rsm.rf.Snapshot(rsm.lastApplied, snapshot)
				rsm.mu.Unlock()
			}
		}
	}
}

func (rsm *RSM) uniqueId() string {
	rsm.mu.Lock()
	defer rsm.mu.Unlock()
	return fmt.Sprintf("%d", rsm.me) + "_" + tester.Randstring(8)
}

// Submit a command to Raft, and wait for it to be committed.  It
// should return ErrWrongLeader if client should find new leader and
// try again.
func (rsm *RSM) Submit(req any) (rpc.Err, any) {

	// Submit creates an Op structure to run a command through Raft;
	// for example: op := Op{Me: rsm.me, Id: id, Req: req}, where req
	// is the argument to Submit and id is a unique id for the op.
	if rsm.killed.Load() {
		return rpc.ErrWrongLeader, nil
	}
	op := Op{
		Req: req,
		Id:  rsm.uniqueId(),
	}

	resCh := make(chan any, 1)
	rsm.mu.Lock()
	rsm.resChMap[op.Id] = resCh
	rsm.mu.Unlock()

	defer func() {
		rsm.mu.Lock()
		delete(rsm.resChMap, op.Id)
		close(resCh)
		rsm.mu.Unlock()
	}()

	_, term, isLeader := rsm.rf.Start(op)
	if !isLeader {
		return rpc.ErrWrongLeader, nil
	}

	ticker := time.NewTicker(time.Millisecond * 10)
	defer ticker.Stop()
	for {
		select {
		case res := <-resCh:
			return rpc.OK, res
		case <-ticker.C:
			currentTerm, _ := rsm.rf.GetState()
			if currentTerm != term {
				return rpc.ErrWrongLeader, nil
			}
		case <-rsm.killedCh:
			return rpc.ErrWrongLeader, nil
		}
	}
}
